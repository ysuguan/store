			...
			<view  class="content">
				...
				<view class="body">
					<scroll-view class="body-scroll" :scroll-y="true">
						...
						<view class="specifications" v-if="comInfo.specifications!==undefined&&comInfo.specifications.length>0">
							<view class="title">规格</view>
							<view class="item-box">
								<u-tag class="option-item" :class="{'option-item-a': index==speciChosen}" 
								v-for="(item, index) in comInfo.specifications" :key="item+index" 
						...
						<view class="number">
							...
							<u-number-box v-model="number" @change="changeNumber" :min="1" :max="numberMax" inputHeight="50" inputWidth="80"></u-number-box>
						</view>
					</scroll-view>
				</view>
				<view class="foot">
...
<script>
	...
	export default {
			...
			comInfo: {
				...
				default: () => { 
					return {
						...
						specifications: ['100片', '50片', '【70g*30片】国产高端', '【35g*100片】进口精品', '【70g*30片】国产高端', '【35g*100片】进口精品', '【70g*30片】国产高端', '【35g*100片】进口精品', '【70g*30片】国产高端', '【35g*100片】进口精品',],
					}
				}
				// required:true,
			}
		...
		methods: {
			...
			//点击确定之后，umask禁止对此组件的操作，向服务器提交数据加入购物车，网络请求结束之后解除umask，关闭当前组件（通过向父组件发送addcart信息）
			addCart() {
				this.maskShow = true;
				setTimeout(() => {
					this.maskShow = false;
					this.$emit('addCart', {res: 'success'});
				}, 1000);
			},
			changeType(index) {
